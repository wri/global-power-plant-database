...
for name, url, code, content in c.execute('SELECT * FROM data'):
	...
	for d in data:
		...
		try:
			...
			continue
		# Handle for inconsistencies in the field names
		field = FIELD_ALIAS.get(field, field)

		# Do some processing on the values to coerce into desirable formats/types
		try:
			new_value = FIELD_PROCESSING[field](value)
		except:
			print 'Unable to process <{0}> for field <{1}>'.format(value, field)
			new_value = value
